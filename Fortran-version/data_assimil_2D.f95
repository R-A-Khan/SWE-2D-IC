program data_assimil_2D
	use FWandBWSolvers
	USE ParametersDA
	USE observations
	USE lib_array
	!USE Line_Minimization
...
double precision :: a, tau_n, tau0
double precision, dimension(Nx,Ny) :: eta_exct0, u, v, eta_optimum
double precision, dimension(Nx,Ny, iter_max) :: steepest_direction
double precision, dimension(Nx,Ny, iter_max+1) :: eta0
double precision, dimension(3*Nx,Ny) :: IC, IC2, IC_bw, IC_iter, IC_iter_opt, IC_line_min
double precision, dimension(2) :: tRange
double precision, dimension(iter_max+1, ntrial) :: cost, grad, error

!double precision, dimension(Nx,Ny) :: test_gradJ_tau
!double precision :: test_cost
...
IF (x0_max > xmax) THEN
  ...
  x0_max = x0_min+(n_obs_x-1)*dmu_x
  WRITE(*,*)  'X-distance between measurement points has been adjusted to fit domain: ', dmu_x
ELSE
   x0_max = x0_min+(n_obs_x-1)*dmu_x
END IF
...
IF (y0_max > ymax) THEN
  ...
  y0_max = y0_min+(n_obs_y-1)*dmu_y
  WRITE(*,*)  'Y-distance between measurement points has been adjusted to fit domain: ', dmu_y
ELSE
   y0_max = y0_min+(n_obs_y-1)*dmu_y
END IF
...
DO jtrial =1, ntrial
    ...
    DO WHILE (norm2(grad_J) >= 1.D-20 .AND. iter<iter_max)
		...
		IC_line_min(1:Nx,:) = eta0(:,:,iter)
		IC_line_min(Nx+1:2*Nx, :) = 0
		IC_line_min(2*Nx+1:3*Nx, :) = 0
		! Optimization for tau

		!print*, " Before Line Minimization tau is = ", tau_n
		!call Lin_Min_lBFGSb( Nx, Ny, Nt, size(x0_inds), T, obs, grad_J,  dt, tRange, IC_line_min, delta_x, &
		!								delta_y, x0_inds, y0_inds , tau_n)
!
		!print*, " After Line Minimization tau is = ", tau_n
        ...
        !print*, iter, tau_n, norm2( grad(iter,jtrial) ), norm2( error(iter, jtrial) )
