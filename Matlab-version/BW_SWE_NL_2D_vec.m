function trend = BW_SWE_NL_2D_vec(t, y, delta_x, delta_y, x0_inds, y0_inds, obs, u_all, v_all, eta_all)
...
trend(Nx+1:2*Nx,:)   = - ( u .* cent_diff_2D_vec(u_adj, delta_x, 1) ...
                         ...
                         + (1 + mid2edge_2D_x_vec(eta) ) .* back_diff_h_2D_vec(eta_adj, delta_x, 1) );
trend(2*Nx+1:3*Nx,:) = - ( v .* cent_diff_2D_vec(v_adj, delta_y, 2) ...
                         ...
                         + (1 + mid2edge_2D_y_vec(eta) ) .* back_diff_h_2D_vec(eta_adj, delta_y, 2) );
% Add observations
