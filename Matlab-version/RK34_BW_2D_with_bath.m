function [t,y] = RK34_BW_2D_with_bath(dt, u0, tRange, trend, h, hy, x0, y0, obs, u_all, v_all, eta_all, bath)
% Usage:
...
%     disp(sprintf('%d',k))
    U0 = y(:,:,k);
    U1 =  alpha(1,1)*U0                 + dt * beta(1,1)*trend(t,U0,h,hy,x0,y0,obs(:,:,k), u_all(:,:,k), v_all(:,:,k), eta_all(:,:,k),  bath);
    %trend(t,U0,h,hy,x0,y0,obs(:,:,k), u_all(:,:,k), v_all(:,:,k), eta_all(:,:,k))
    U2 =  alpha(2,2)*U1                 + dt * beta(2,2)*trend(t,U1,h,hy,x0,y0,obs(:,:,k), u_all(:,:,k), v_all(:,:,k), eta_all(:,:,k),  bath);
    U3 =  alpha(3,1)*U0 + alpha(3,3)*U2 + dt * beta(3,3)*trend(t,U2,h,hy,x0,y0,obs(:,:,k), u_all(:,:,k), v_all(:,:,k),  eta_all(:,:,k), bath);
    U4 =  alpha(4,4)*U3                 + dt * beta(4,4)*trend(t,U3,h,hy,x0,y0,obs(:,:,k), u_all(:,:,k), v_all(:,:,k),  eta_all(:,:,k), bath);
    y(:,:,k+1) = U4;
end